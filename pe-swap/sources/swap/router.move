module equity::router {
    ...
    public entry fun create_pair<X, Y>(
        ...
        if (swap_utils::sort_token_type<X, Y>()) {
            swap::create_pair<X, Y>(sender,expA,expB);
        } else {
            swap::create_pair<Y, X>(sender,expB,expA);
        };
    }
    ...
    public entry fun swap_exact_output<X, Y>(
        ...
        let x_in = if (swap_utils::sort_token_type<X, Y>()) {
            let (rin, rout, expA,expB,_) = swap::token_reserves<X, Y>();
            let amount_in = swap_utils::get_amount_in(y_out, rin, rout,expA,expB);
            swap::swap_x_to_exact_y<X, Y>(sender, amount_in, y_out, signer::address_of(sender))
        } else {
            let (rout, rin, expB,expA,_) = swap::token_reserves<Y, X>();
            let amount_in = swap_utils::get_amount_in(y_out, rin, rout,expA,expB);
    ...
    fun get_amount_in_internal<X, Y>(is_x_to_y:bool, y_out_amount: u64): u64 {
        if (is_x_to_y) {
            let (rin, rout, expA,expB,_) = swap::token_reserves<X, Y>();
            swap_utils::get_amount_in(y_out_amount, rin, rout,expA,expB)
        } else {
            let (rout, rin, expB,expA,_) = swap::token_reserves<Y, X>();
            swap_utils::get_amount_in(y_out_amount, rin, rout,expA,expB)
        }
    ...
    fun swap_exact_output_double_internal<X, Y, Z>(
        ...
        let y_out = if (second_is_y_to_z) {
            (rout, rin, expB,expA,_) = swap::token_reserves<Y, Z>();
            swap_utils::get_amount_in(z_out, rin, rout,expA,expB)
        }else {
            (rout, rin, expB,expA,_) = swap::token_reserves<Z, Y>();
            swap_utils::get_amount_in(z_out, rin, rout,expA,expB)
        };
        let x_in = if (first_is_x_to_y) {
            (rout, rin, expB,expA,_) = swap::token_reserves<X, Y>();
            swap_utils::get_amount_in(y_out, rin, rout,expA,expB)
        }else {
            (rout, rin, expB,expA,_) = swap::token_reserves<Y, X>();
            swap_utils::get_amount_in(y_out, rin, rout,expA,expB)
        };
    ...
    fun swap_exact_output_triple_internal<X, Y, Z, A>(
        ...
        let z_out = if (third_is_z_to_a) {
            (rout, rin, expB,expA,_) = swap::token_reserves<Z, A>();
            swap_utils::get_amount_in(a_out, rin, rout,expA,expB)
        }else {
            (rout, rin, expB,expA,_) = swap::token_reserves<A, Z>();
            swap_utils::get_amount_in(a_out, rin, rout,expA,expB)
        };
    ...
    fun swap_exact_output_quadruple_internal<X, Y, Z, A, B>(
        ...
        let a_out = if (fourth_is_a_to_b) {
            (rout, rin, expB,expA,_) = swap::token_reserves<A, B>();
            swap_utils::get_amount_in(b_out, rin, rout,expA,expB)
        }else {
            (rout, rin, expB,expA,_) = swap::token_reserves<B, A>();
            swap_utils::get_amount_in(b_out, rin, rout,expA,expB)
        };
